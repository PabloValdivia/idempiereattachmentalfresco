...
import org.compiere.model.MAttachmentEcosoft;
import org.compiere.model.MAttachmentEntryEcosoft;
 ...
 */
public final class AttachmentEcosoft extends CDialog
	implements ActionListener
{
	 ...
	 */
	public AttachmentEcosoft(Frame frame, int WindowNo, int AD_Attachment_ID,
		int AD_Table_ID, int Record_ID, String trxName)
	{
		...
		if (AD_Attachment_ID == 0)
			m_attachment = new MAttachmentEcosoft (Env.getCtx(), AD_Table_ID, Record_ID, trxName);
		else
			m_attachment = new MAttachmentEcosoft (Env.getCtx(), AD_Attachment_ID, trxName);
		loadAttachments();
		//
		try
		{
	...
	/** Attachment				*/
	private MAttachmentEcosoft		m_attachment;
	/** Change					*/
	private boolean			m_change = false;
	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(AttachmentEcosoft.class);
	...
	// CMIS by KTU
	private CButton bOpenCMIS = new CButton();
	// --
	...
	void staticInit() throws Exception
	{
		...
		cmisViewerURL = client.get_ValueAsString("CMISViewerUrl");
		// --
		...
		// CMIS by KTU
		if (isCmis) toolBar.add(bOpenCMIS);
		// --
		...
		bOpenCMIS.setIcon(Env.getImageIcon("Online24.gif"));
		bOpenCMIS.setMargin(new Insets(0, 2, 0, 2));
		bOpenCMIS.setToolTipText(Msg.getMsg(Env.getCtx(), "Open in CMIS"));
		bOpenCMIS.addActionListener(this);
		// --
		//
	...
	{
		MAttachmentEntryEcosoft entry = m_attachment.getEntry(index); 
	...
	/**
	 * Open attachment via CMIS's Browser
	 * @return
	 */
	private boolean openAttachmentCMIS ()
    {
        int index = cbContent.getSelectedIndex();
        MAttachmentEntryEcosoft docEntry = m_attachment.getEntry(index);
        if (docEntry == null)
            return false;
